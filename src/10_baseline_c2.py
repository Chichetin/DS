"""
Шаг D2: LGBM на C2 (C1 + history-фичи 3 уровней).

Идентично 06_baseline_lgbm.py но читает c2_*.parquet и пишет в artifacts/baseline_c2/.
"""
...
ART = ROOT / "artifacts" / "baseline_c2"
...
def load_pandas(name: str, has_target: bool) -> pd.DataFrame:
    df = pl.read_parquet(FEAT / f"c2_{name}.parquet").to_pandas()
...
def main() -> None:
    t0 = time.time()
    log(f"LGBM C2 baseline. lightgbm={lgb.__version__}, polars={pl.__version__}")
