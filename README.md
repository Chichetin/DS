...
> Хакатон ITMO × Avito, команда **«ИИ в массы»**. Решение I этапа, дедлайн 10.05.2026.

## TL;DR

**Задача.** Бинарная классификация: для каждой пары `(item_id, deliveryorder_id)` в test-выборке (4 030 995 строк за 01-10.10.2025) предсказать `is_return` — вернёт покупатель товар или нет.

**Данные.** 5 таблиц от Avito за 01.07-10.10.2025: 39M заказов (33M после дедупа), 20.6M объявлений, 11.7M пользователей, 30M платежей. Сырьё + промежуточные витрины ≈ 30 ГБ, всё считалось локально на 16 ГБ RAM.

**Подход.** Time-split валидация (holdout — последние 10 дней train, 2025-09-21..09-30, ~3.7M строк без сэмплинга), четыре итерации фич C1→C4 с замером прироста на каждом шаге, LightGBM + CatBoost, блендинг и калибровка порога по F1. Отдельный акцент — на защите от лика: все поля, заполняемые после `order_create_date`, вырезаны, история покупателя/продавца/товара считается строго по окну `[-∞, дата заказа − 1]`.

**Результат.** ROC-AUC **0.805** на honest holdout при базовой частоте возвратов 4.56% (сильный дисбаланс). Ключевой прирост дал блок исторических фич C2: **+0.028 AUC, +0.050 F1** над базой.

**Мой вклад.** Весь ML-пайплайн в этом репозитории: препроцессинг и дедуп на DuckDB, дизайн фич C1-C4, обучение и тюнинг LGBM/CatBoost, блендинг, сборка сабмита.

### Метрики на holdout (2025-09-21..09-30)
...
| **🏁 LGBM full (на ПОЛНОМ train, 33M строк) — финальная модель** | **0.80496** | 0.27980 | 0.226 | 0.367 |

> **Важно:** ROC-AUC и F1 оцениваются на **полном holdout** (~3.7M строк за 09-21..09-30, без сэмплинга) — никакого сэмплирования на валидации, метрики стабильны между прогонами.

> **Что не успели до дедлайна:** CatBoost на 22% сэмпле (`src/26`) и финальный блендинг LGBM-full + CatBoost-22 (`src/27`) — код написан и готов к запуску, но прогон (~3-4 ч) не уложился в оставшееся время. Лучший подтверждённый результат — LGBM-full. Ожидаемый эффект от блендинга по опыту 15% сэмпла — порядка +0.0003 AUC, то есть на итог он бы почти не повлиял.
...
### Blend (написан, но в финал не попал)
...
Best на 15% сэмпле: **α = 0.30 (LGBM-вес), threshold ≈ 0.77, AUC 0.80492, F1 0.28199**. Прогон на 22% сэмпле не успели завершить — см. оговорку в TL;DR.
...
### Не успели в срок (код готов, нужен только прогон)

- **CatBoost-22 + финальный блендинг** — `src/26`, `src/27`. ~3-4 ч на 16 ГБ RAM.
- **Sanity-проверки сабмита:** target rate ≈ 4.5% (см. `metrics.txt`), нет NaN, все 4 030 995 строк уникальны по `(deliveryorder_id, item_id)`.
- **Перепроверить порог.** Стандартно ловим F1, но возможно устойчивее зафиксировать 0.5 или взять середину между F1-thr и Youden.
...
Решение готовилось командой «ИИ в массы» в рамках хакатона ITMO × Avito. Код в этом репозитории — моя часть работы (ML-пайплайн целиком).
